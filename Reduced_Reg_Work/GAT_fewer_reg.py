...
os.environ['WANDB_NOTEBOOK_NAME']="GAT_fewer_reg.ipynb"
...
def train(model, optimizer, criterion, w_decay, threshold, train_loader, valid_loader, parameters, test_loader=False, testing=False, n_epochs=100):
    ...
    heads = parameters[4]
    if matrixprofile:
        filename = f'Reduced_reg_models/GAT_Models_MP/threshold_{threshold}/lr{lr}_hc{hidden_channels}_nl{num_layers}_d{dropout}_epochs{n_epochs}_heads{heads}_wdecay{w_decay}_w{weight}.png'
    else:
        filename = f'Reduced_reg_models/GAT_Models/threshold_{threshold}/lr{lr}_hc{hidden_channels}_nl{num_layers}_d{dropout}_epochs{n_epochs}_heads{heads}_wdecay{w_decay}_w{weight}.png'
...
for params in param_combinations:
    if matrixprofile:
        filename = f'Reduced_reg_models/GAT_Models_MP/threshold_{threshold}/lr{params["learning_rate"]}_hc{params["hidden_channels"]}_nl{params["num_layers"]}_d{params["dropout_rate"]}_epochs{n_epochs}_heads{params["heads"]}_wdecay{params["weight_decay"]}_w{weight}.png'
    else:
        filename = f'Reduced_reg_models/GAT_Models/threshold_{threshold}/lr{params["learning_rate"]}_hc{params["hidden_channels"]}_nl{params["num_layers"]}_d{params["dropout_rate"]}_epochs{n_epochs}_heads{params["heads"]}_wdecay{params["weight_decay"]}_w{weight}.png'
